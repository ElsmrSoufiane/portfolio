  ...
  <div id="page_741926" class="min-h-screen overflow-hidden">
    ...
    <section id="home" class="relative mx-auto grid max-w-[1280px] gap-12 px-6 pb-24 pt-16 lg:grid-cols-[1.05fr_.95fr] lg:px-12 lg:pb-32 lg:pt-24">
      ...
      <div id="hero_copy_306728" class="relative z-10 rise">
        ...
        <span class="mx-2 font-bold"> {{ __('Build useful things.') }}</span> 
          <span class="text-[#4A9EE8] font-bold ">{{ __('Ship them beautifully.') }}</span>
        </h1>
        <p id="hero_description_285716" class="mt-8 max-w-xl text-lg font-light leading-8 text-[#8AAEC8]">
          {{ __('I design and build focused digital products with Laravel, TallStack, Filament, and modern DevOps workflows. Explore my work, learn with me, or bring an idea to life.') }}
        </p>
        ...
        <div id="workspace_card_816394" class="relative w-full max-w-md rotate-2 rounded-2xl border border-[#2A4060] bg-[#141920] p-3 soft transition duration-700 hover:rotate-0 hover:scale-[1.02]">
          <div id="workspace_inner_527681" class="rounded-xl border border-[#1E3050] bg-[#1A2130] p-5">
            <div id="workspace_header_391752" class="mb-8 flex items-center justify-between">
              <span class="text-sm text-[#8AAEC8]">{{ __('Developer workspace') }}</span>
              <span class="rounded-full bg-[#3DCC8E]/10 px-2 py-1 text-xs text-[#3DCC8E]">{{ __('Active') }}</span>
            </div>
            <div id="workspace_block_648215" class="mb-8">
              <span class="text-sm text-[#506070]">{{ __('Current focus') }}</span>
              <div class="display mt-2 text-4xl font-bold">
                {{ __('Shipping products') }}
                <span class="ml-2 text-sm font-normal text-[#4A9EE8]">{{ __('on track') }}</span>
              </div>
            </div>
            <svg id="workspace_chart_927416" viewBox="0 0 500 150" class="h-36 w-full" aria-label="{{ __('Progress chart') }}">
            ...
            <div id="workspace_metrics_581629" class="grid grid-cols-2 gap-3">
              <div id="skills_metric_746281" class="rounded-lg bg-[#141920] p-3">
                <span class="text-xs text-[#506070]">{{ __('Stack') }}</span>
                <strong class="display mt-1 block text-xl">Laravel</strong>
              </div>
              <div id="delivery_metric_295814" class="rounded-lg bg-[#141920] p-3">
                <span class="text-xs text-[#506070]">{{ __('Delivery') }}</span>
                <strong class="display mt-1 block text-xl">{{ __('Reliable') }}</strong>
              </div>
            </div>
          </div>
        </div>
   ...
   <div id="hero_stats_921564" class="mt-5 grid   grid-cols-3 gap-5 border-t border-[#1E3050] pt-2 place-items-center  w-full">
    ...
    <section id="projects" class="mx-auto max-w-[1280px] px-6 py-20 lg:px-12">
      <div id="projects_heading_517649" class="mb-10 flex items-end justify-between">
        ...
        <a id="projects_contact_link_381926" href="https://wa.me/212766548709" target="_blank" rel="noopener" class="hidden text-sm text-[#8AAEC8] md:block">
          {{ __('Need something similar?') }}
          <span class="text-[#4A9EE8]">{{ __('Let\'s talk') }} →</span>
        </a>
      </div>
    ...
    <section id="videos" class="mx-auto max-w-[1280px] px-6 py-20 lg:px-12">
      <div id="videos_header_618294" class="flex flex-col justify-between gap-8 md:flex-row md:items-end">
        <div id="videos_title_group_752681">
          <span class="text-xs uppercase tracking-[.2em] text-[#4A9EE8]">02 / {{ __('Tutorials') }}</span>
          <h2 class="display mt-3 text-4xl font-bold">{{ __('Learn by shipping.') }}</h2>
        </div>
        <a id="videos_all_link_481625" href="/blog" wire:navigate class="text-sm text-[#4A9EE8]">{{ __('View all videos') }} →</a>
      </div>
      <div id="videos_grid_739214" class="mt-10 grid gap-5 md:grid-cols-2">
        ...
        <article id="video_{{$blog->id}}" class="group flex gap-5 rounded-xl border border-[#1E3050] bg-[#141920] p-4 transition duration-700 hover:border-[#4A9EE8]">
          ...
          <div id="video_{{$blog->id}}_copy">
            <span class="text-xs text-[#506070]">{{ __('Duration') }} {{ $blog->duration }}  <br>   {{ __('Views') }} {{ $blog->views }}  <br>   {{ $blog->tags->pluck('name')->implode(' · ') }}</span>
            <h3 class="display mt-2 text-xl font-bold">{{ $blog->title }}</h3>
          </div>
        </article>
        </a>
        @empty
        <p class="col-span-full rounded-xl border border-dashed border-[#1E3050] py-16 text-center text-sm text-[#506070]">{{ __('No videos published yet — check back soon.') }}</p>
        @endforelse
      </div>
    </section>
    ...
    <section id="about" class="border-t border-[#1E3050] bg-[#141920]">
      <div id="about_inner_752416" class="mx-auto max-w-[1280px] px-6 py-20 lg:px-12">
        <span id="about_eyebrow_618295" class="about-reveal text-xs uppercase tracking-[.2em] text-[#4A9EE8]">03 / {{ __('About') }}</span>
        <div id="about_content_491726" class="about-reveal mt-5 grid gap-8 lg:grid-cols-[.8fr_1.2fr]">
          <h2 id="about_heading_826415" class="display text-4xl font-bold">{{ __('Code, products, and ideas that move forward.') }}</h2>
          <p id="about_description_739182" class="max-w-2xl text-lg leading-8 text-[#8AAEC8]">{{ __('I\'m a full-stack developer working with PHP, Laravel, TallStack, Filament, and DevOps tools. Alongside building reliable products, I bring nine months of digital marketing experience to help ideas reach the right audience.') }}</p>
        </div>
      </div>
    </section>
